...
// edit  "../ogs-internal/ows" instances
	...
	constructor() {
		...
		this.viewLayerTitle = "Fibre Connection Location"; // Must match title of 'iss:ISS_MFN_CONNECTION_LOCATION' in appConfig.js
		this.canEdit = false;
		this.busy = false;
		this.addInUse = false;
	...
	* @param(string,string,string,string,string) Conduit_id, Location Name, Location Type, Albers X, Albers Y,
	* @returns(geometry) 
	* Function to snap a given point to the nearest point on a Conduit	
	*/

	insertConnectionLocation(conduitId,locName,locType,locAddress,locNotes,albers_x,albers_y) {
		// create XML insert
		var data = `
	 ...
	 */
	reverseGeocode(coords,element,type) {
		const geoXY = ol.proj.transform(coords, app.map.getView().getProjection().getCode(), "EPSG:4326");
		if (type=="latLong") {
	 ...
	 * @param (feature,feature,feature) conduit in question, location to be added, cabinet (optional)
	 * @returns () nothing
	...
	confirmPoint(conduitFeature,newPoint,cabinet) {
		...
		const ptCoords = newPoint.getGeometry().getCoordinates();			
		const albersXY = ol.proj.transform(ptCoords, app.map.getView().getProjection(), "EPSG:3005");
		const albers_x = albersXY[0].toFixed(3);
		const albers_y = albersXY[1].toFixed(3);	
// Get info on existing cabinet if required
		...
		// show found conduit (TO DO, populate with all found
		$("#mfn-connection-location-form").find("[key='CONDUIT_ID']" ).append(new Option(conduitID,conduitFeature.properties.CONDUIT_NAME));		
		// set up form buttons
		...
		$("#mfn-reverse-geocode1").unbind().click(function(){
			app.plugins.MFNaddConnection.reverseGeocode(ptCoords,$("#mfn-connection-location-form").find("[key='LOCATION_ADDRESS']"),"sites");
		});
		$("#mfn-reverse-geocode2").unbind().click(function(){
			app.plugins.MFNaddConnection.reverseGeocode(ptCoords,$("#mfn-connection-location-form").find("[key='LOCATION_ADDRESS']"),"intersections");
		});
		$("#mfn-reverse-geocode3").unbind().click(function(){
			app.plugins.MFNaddConnection.reverseGeocode(ptCoords,$("#mfn-connection-location-form").find("[key='LOCATION_ADDRESS']"),"latLong");
		});
		...
		$("#mfn-save-location").on("click",function(e){
			e.stopPropagation(); // Stops function from firing
			e.preventDefault();  // more than once.
			...
			if (locName.length > 1) { // name is mandatory
				$("#mfn-connection-location-form").find("[key='LOCATION_NAME']").removeClass("mfn-validation-error");
				app.plugins.MFNaddConnection.insertConnectionLocation(conduitID,locName,locType,locAddress,locNotes,albers_x,albers_y);
				$("#mfn-connection-location-form").hide();	
				app.plugins.MFNaddConnection.addPointCleanup();
			} else {
	 ...
	 * @param (array[2],object) coordinates of map click intended to add a Connection Location.
	 * @returns () nothing
	 * Function to add a Connection Location on the nearest conduit.
	 */
	processAddPointClick(pointFeature,cabinet) {
		// Get info on the Conduit so as to snap to it.
		
		const albersXY = ol.proj.transform(pointFeature.getGeometry().getCoordinates(), app.map.getView().getProjection(), "EPSG:3005");
		const albers_x = albersXY[0].toFixed();
		const albers_y = albersXY[1].toFixed();
		const factor = app.map.getView().getResolution() * 100;
			...
			if (response.features.length < 1) {
				...
				const callbackGotSnappedPoint = function(conduitPoint) {
					if (conduitPoint.features.length < 1) {
						...
						$("#popup-content").html("No conduit found "+exception);
						$("#map #popup").show().delay(3000).fadeOut();
						app.plugins.MFNaddConnection.addPointCleanup();
					} else { // TO DO, SEND LIST OF CONDUITS IF MULTIPLE FOUND AND ALLOW USER TO PICK ONE
						app.plugins.MFNaddConnection.confirmPoint(conduitFeature,pointFeature,cabinet);
					}										
				}
...
*/
	...
	mapClickFunction(e){
		...
		if (app.plugins.MFNaddConnection.busy) {
			...
			app.plugins.MFNaddConnection.processAddPointClick(pointFeature);
		}
	}
    /**
	...
	displayLhsConnection(feat) {
		...
		if 	((app.plugins.MFNaddConnection.thisLocation.location_type!="Junction Box") && (app.plugins.MFNaddConnection.thisLocation.location_type!="Splice Case")) {
			// no devices in splices
			$(conn).find(".mfn-lhs-port-number").show();
			$(conn).find(".mfn-device-name").show();
		}
	...
	displayRhsConnection(feat,isNew) { 
		...
		if 	((app.plugins.MFNaddConnection.thisLocation.location_type!="Junction Box") && (app.plugins.MFNaddConnection.thisLocation.location_type!="Splice Case")) {
			// no devices in splices
			$(conn).find(".mfn-rhs-port-number").show();
			$(conn).find(".mfn-device-name").show();
		}
